...
for l in range (7,8): #len(data_input.index)
    ...
    params = {}
    for kw in keywords:
        slope, intercept, r, p, se = linregress(x, output[kw])
    ...
    # Create dataframe to store residuals vs baseline (regression)
    residuals = pd.DataFrame().reindex_like(output)
    residuals['week'] = output['week']
    residuals['x'] = output['x']
    
    # Calculate residuals vs baseline for each keyword
    for col in tqdm(keywords):
        for t in residuals.index:
            intercept, slope = params[col]
            residuals.loc[t,col] = output.loc[t,col] - (intercept + slope*output.loc[t,'x'])
    
    # AVERAGE RESIDUALS PER WEEK
    # Average residual for each week
    residuals_avg = residuals.drop('x', axis=1).groupby('week').mean()
    ...
    for col in tqdm(keywords):
        for t in output_adj.index:
            week = output_adj.loc[t,'week']
            adj = residuals_avg.loc[week,col]
            output_adj.loc[t,col] = output.loc[t,col] + adj
